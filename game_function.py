...
def check_keydown_events (event, ai_setings, screen, ship, bullet):
...
def check_events(ai_setings, screen, ship, bullet):
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            sys.exit()
        elif event.type == pygame.KEYDOWN:
            check_keydown_events (event, ai_setings, screen, ship, bullet)
...
def update_screen(ai_setings, screen, stats, ship, bullet, aliens, play_button):
...
def update_aliens (ai_setings, stats, screen, ship, aliens, bullet):
    check_fleet_edges (ai_setings, aliens)
    aliens.update ()
    if pygame.sprite.spritecollideany(ship, aliens):
        ship_hit (ai_setings, stats, screen, ship, aliens, bullet)
    check_aliens_bottom (ai_setings, stats, screen, ship, aliens, bullet)

def ship_hit (ai_setings, stats, screen, ship, aliens, bullet):
...
def update_bullet(ai_setings, screen, ship, bullet, aliens):
    bullet.update ()
    for bullets in bullet.copy():
        if bullets.rect.bottom <=0:
            bullet.remove (bullets)
    check_bullet_alien_collisions (ai_setings, screen, ship, aliens, bullet)

def check_bullet_alien_collisions (ai_setings, screen, ship, aliens, bullet):
